...
class AccountServiceImplTest {
    ...
    private final CustomerClientPort customerClient = mock(CustomerClientPort.class);
    ...
    void createShouldSaveAccountSuccessfully() {
        ...
        Customer customer = activeCustomer("customer-001");

        when(customerClient.getById("customer-001")).thenReturn(Mono.just(customer));
        when(repository.findByIdAndType("customer-001", AccountType.SAVINGS)).thenReturn(Mono.empty());
        ...
        verify(customerClient).getById("customer-001");
        verify(repository).findByIdAndType("customer-001", AccountType.SAVINGS);
    ...
    void updateShouldSaveUpdatedAccountSuccessfully() {
        ...
        Account expectedAccountToSave = new Account(
                id,
                "customer-001",
                "ACC-001",
                AccountType.SAVINGS,
                ...
                accountToUpdate.updatedAt(),
                accountToUpdate.closedAt(),
                true,
                "ACTIVE"
        );

        when(repository.findById(id)).thenReturn(Mono.just(existingAccount));
        when(repository.save(expectedAccountToSave)).thenReturn(Mono.just(expectedAccountToSave));

        StepVerifier.create(service.update(id, accountToUpdate))
                .expectNext(expectedAccountToSave)
                .verifyComplete();

        verify(repository).findById(id);
        verify(repository).save(expectedAccountToSave);
    }
    ...
    private Customer activeCustomer(String id) {
        return new Customer(
                id,
                "PERSONAL",
                "DNI",
